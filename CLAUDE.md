...
**小生物不是「畫小一點的動物」——它有自己的最小可讀尺寸。**
松鼠畫了**八次**，前七次全部失敗，而共同原因不是造型是**尺寸**。
20–25 px 下試過：極座標畫弧（糊成一團）、逐列明寫加描邊
（2 px 的尾巴加兩側描邊變 4 px，弧線繞回來把中間封成一個圈）、
不描邊的實心剪影（尾巴和身體同色融成一坨）、尾巴改成往後拖的羽狀（像石頭）、
尾巴與身體用明度分開（有幫助但仍讀不出來）。
**放大到 40×36（狗的 62%）一次就對。**
最小可讀尺寸由**最細的辨識特徵**決定——松鼠是「尾巴與身體之間的那道縫」。
小鳥 16×12 一次就成，因為牠每個部位都是凸形，沒有「必須和自己保持距離」的部位。

**放大檢查和實際尺寸檢查會給出不同答案，兩個都要看。**
松鼠的尾巴第一版畫成中空的（想表現捲曲的層次），8× 放大看是尾巴，
貼進房間 1:1 看是甜甜圈。
