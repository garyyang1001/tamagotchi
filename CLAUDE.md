...
- **畫面裡不要放虛擬按鍵。** 三顆實體鍵在螢幕下方的外殼上，畫面裡再放框等於畫兩次。
  房間是**滿版 320×240**，動作選單是只在需要時疊上來的浮層。
  地面線必須落在浮層上緣之上，否則選單一開就把角色的下半身蓋掉。
...
**保護之前要先量哪個顏色佔多數，不要憑印象挑。** 飯碗裡的飼料被量化吸成紅色，
我第一次保護的是 `#663C1B`——但那只是**深色顆粒**，只佔食物的 25%，
主體是 `#B77133` 的淺褐。保護了少數派等於沒保護。兩個都保護才對。
...
牆、地板、窗框、碗這種**場景**元素的價值在精確（接縫要對齊、地板不能抖），
AI 在那裡只會多一輪 28,000 色的重映射——那些歸工具。
動作選單的圖示（飯碗、球、手、澡盆、馬桶）的價值在**一眼認得出來而且討喜**，
那正是模型擅長的——手繪版能認得出來但**平**，單色塊、沒有份量。
2026-08-03 改走 AI 管線之後明顯好看。
10×10 的需求圖示仍然手繪：那個尺寸低於模型的有效控制解析度。

**生物走 AI，幾何走工具——這條沒有例外，連 40 px 的松鼠也是。**
松鼠先用手繪像素試了**八次**全部失敗。前七次共同的原因是尺寸
（20–25 px 沒有空間讓尾巴離開身體：弧線一繞回來就被描邊封成一個圈、
實心又和身體融成一坨），第八次放大到 40×40 才「看得出是松鼠」——
但和模型畫的並排，剪影還是粗糙很多。
**我違反了自己訂的規則 2 才浪費那八輪。** 房間、碗、床是幾何，松鼠是生物。

順帶量到的兩件事仍然成立：
小生物有自己的**最小可讀尺寸**，由最細的辨識特徵決定（松鼠是「尾巴與身體之間的那道縫」）；
**放大檢查和實際尺寸檢查會給出不同答案，兩個都要看**
（中空的尾巴 8× 看是尾巴，1:1 看是甜甜圈）。

**`automap` 的 `sat` 是 `max(RGB) − min(RGB)`（0–255），不是 HSV 的百分比。**
小鳥的奶油腹 HSV 飽和 26%，但 automap 算出來是 66。
照百分比直覺把 `sat_max` 寫成 60，整片腹部就被踢出奶油族、掉進橘族——
實測腹部整個變成橘色。**改調色盤的 automap 門檻之前先用同一個公式量一遍。**

**色相族群要留亮度下限，而且要涵蓋所有色相。** 同一次踩到兩個：
描邊裡有一批暗像素色相落在 40°（`#0C0A06` 亮度只有 10），
奶油族沒有亮度下限就把它們判成腹部——描邊整圈變奶油、腹部反而變黑；
橘喙飽和 239，藍族色相不合、奶油族飽和上限擋掉，**不屬於任何一族**，
掉進 fallback 被判成 outline——橘喙整個變黑。
...
**量對比要量「最終真的疊在一起的那兩層」，不是各自假設的那一層。**
提示圖示做成深色，因為它要畫在淺色的牆上（實測距離 305，很安全）；
渲染層另外墊了一塊深色底板，因為擔心圖示壓在牆和地毯上會糊。
兩邊各自量對，**合起來是深色圖示壓在深色底板上，距離只剩 44，腳印幾乎看不見**。
這種錯特別容易發生在「資產」和「怎麼畫」由不同人（或不同輪）決定的時候——
資產那一輪量的是設計意圖的背景，渲染那一輪量的是它以為的背景，沒有人量真正的組合。
**最後一定要看實際畫出來的那一格。**
