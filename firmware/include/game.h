...
/* 狗從哪一側進出房間。**-1 = 門在站位的左邊。**
   2026-08-02 從 +1 改成 -1：門畫在 x6-46、狗站在 x40（見 specs/scene.json），
   門在左邊。原本的 +1 讓狗離場時往**反方向**走，而註解還寫著「+1 = 門在右邊」——
   常數和它的註解一起錯了，兩邊互相背書所以一直沒被發現。 */
#define DOOR_SIDE (-1)

/* 離場要走多遠才算真的出畫面。**不能用 CHAR_X_RANGE**：
   那是閒置走動的範圍（±24），而狗的影格有 64 px 寬——移動 24 px 之後
   離場的和到場的兩隻仍然重疊八成，畫面上是一團糊掉的狗。
   這個值必須 ≥ 狗的站位 x + 影格寬，render.c 有 _Static_assert 在守。 */
#define EXIT_X_RANGE 104
